...
module.exports = class Job { 
  ...
  static async insert ({  
    ...
    const { rows } = await pool.query(
      ...
      RETURNING *
      `, [user_id, fav, remote, zipcode, wishlist, applied, phone_screen, interviewed, take_home, technical_interview, offer, rejected, accepted, url, description, notes, contact]      
    );
  ...
  static async updateJob(id, attributes){
    ...
    const { rows } = await pool.query(`
      UPDATE jobs SET  
        ...
        WHERE id = $18 
        RETURNING *
      `, [fav, remote, zipcode, wishlist, applied, phone_screen, interviewed, take_home, technical_interview, offer, rejected, accepted, url, description, notes, contact, last_updated, id]   
    );
