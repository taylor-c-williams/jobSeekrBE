...
module.exports = class Job { 
  ...
  static async insert ({  
    ...
    const { rows } = await pool.query(
      ...
      RETURNING *
      `, [user_id, fav, remote, zipcode, applied, phone_screen, interviewed, take_home, offer, rejected, accepted, url, description, notes, contact]      
    );
  ...
  static async updateJob(id, attributes){
    ...
    const { rows } = await pool.query(`
      UPDATE jobs SET  
        ...
        WHERE id = $16 
        RETURNING *
      `, [fav, remote, zipcode, applied, phone_screen, interviewed, take_home, offer, rejected, accepted, url, description, notes, contact, last_updated, id]   
    );
  ...
  // Get Jobs where Interviewed = True
  static async getInterviewedJobs(id){
    const { rows } = await pool.query(`
    SELECT * 
    FROM jobs
    WHERE id = $1 and interviewed = true
    `, [id]);
    return rows.map((row) => new Job(row));
  }
